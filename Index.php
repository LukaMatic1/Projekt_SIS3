      ...
      <div class="col-md-10">
        ...
        <?php
          ...
          if(isset($_GET['addToCart'])){
            ...
            $getIpAddress = getIPAddress();
            $getProductId=$_GET['addToCart'];           //(Up_Id)
            $selectQuery="SELECT * FROM `Vozicek` WHERE ip_address='$getIpAddress' AND Izd_Id=$getProductId";
            $resultQuery=mysqli_query($con,$selectQuery);
            $numOfRows=mysqli_num_rows($resultQuery);
              if($numOfRows>0){
                echo "<script>alert('Izdelek se že nahaja v vozičku')</script>";
                echo "<script>window.open('Index.php','_self')</script>";
              }else{                                        //(Up_Id)
                $insertQuery="INSERT INTO `Vozicek` (Izd_Id,ip_address,Kolicina) VALUES ($getProductId,'$getIpAddress',0)";
